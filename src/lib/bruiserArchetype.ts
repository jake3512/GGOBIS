...
//
// Fighter 태그는 서포터(Thresh)·정글 암살자(Rengar 등)·마법사 하이브리드
// (Diana, Elise) 등 "브루저"라 부르기 애매한 챔피언까지 폭넓게 걸쳐 있어서,
// 다른 두 표와 달리 태그 하나로 대상을 다 잡지 않고 실제로 "브루저"로 통하는
// 핵심 챔피언 풀만 골라 담았습니다 — 나머지 Fighter 태그 챔피언은 표에 없는
// 것과 동일하게 조용히 빠집니다(억지로 채우지 않음).
//
...
// 경우)에만 붙입니다.
//
// "유틸"(utility)은 딜/생존이 아니라 CC·이동기·군중 제어처럼 전투 판을
// 바꾸는 능력이 킷의 핵심인 경우에 붙입니다 — 위 세 속성(흡혈/공속/탱킹)이
// 전부 딜을 넣거나 버티는 방식에 대한 것과는 결이 다른 네 번째 축입니다.

export type BruiserAttribute = "lifesteal" | "attackSpeed" | "tanky" | "utility";
...
export const BRUISER_ARCHETYPES: Record<string, BruiserArchetype> = {
  ...
  Gnar: { attributes: ["utility"] },
  Gragas: { attributes: ["tanky"] }, // 체력 비례 킷 + 술통 자힐로 두꺼운 몸싸움
  Hecarim: { attributes: ["utility", "tanky"] },
  Kayn: { attributes: ["utility"] },
  Kled: { attributes: ["tanky"] },
  LeeSin: { attributes: ["utility"] },
  Nunu: { attributes: ["utility"] },
  RekSai: { attributes: ["utility", "tanky"] },
  Riven: { attributes: ["utility"] },
  ...
  Volibear: { attributes: ["lifesteal", "tanky"] },
  XinZhao: { attributes: ["lifesteal", "utility"] },
  Yasuo: { attributes: ["attackSpeed"] },
  Yorick: { attributes: ["tanky", "utility"] },
};
