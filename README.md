...
- **픽 추천**: 픽창에서 내가 픽할 포지션을 고르고, 상대가 같은 라인에 이미 픽한 챔피언과(있다면) 우리팀 원거리 딜러를 입력하면 좋은 픽을 추천합니다. 내부적으로 라인 카운터/바텀 듀오와 **같은 데이터**를 재사용합니다 — 상대 라이너가 있으면 그 챔피언을 상대로 승률 높은 챔피언 목록(=카운터)을, 서포터를 픽하는 중이고 우리팀 원거리 딜러가 정해졌으면 그 원거리 딜러와 시너지 좋은 서포터 목록을 보여주고, 둘 다 있으면 두 지표를 평균 내 "둘 다 좋은 픽"도 따로 보여줍니다. 시너지 추천은 서포터를 픽할 때만 제공됩니다 — 확인된 사이트 데이터가 "원거리 딜러 → 시너지 좋은 서포터" 방향의 목록만 있고, 반대 방향(서포터 알고 있을 때 원거리 딜러 추천)이나 탑/정글/미드의 팀 시너지 데이터는 확인된 소스가 없기 때문입니다.
